...
export default function CreateTest() {
  ...
  const Submit = async () => {
    const hasEmptyObjectL1 = listenP1.filter(
      (obj) => Object.keys(obj).length === 0
    );
    const hasEmptyObjectL2 = listenP2.filter(
      (obj) => Object.keys(obj).length === 0
    );
    const hasEmptyObjectL3 = listenP3.filter(
      (obj) => Object.keys(obj).length === 0
    );
    const hasEmptyObjectL4 = listenP4.filter(
      (obj) => Object.keys(obj).length === 0
    );
    const hasEmptyObjectR1 = readP1.filter(
      (obj) => Object.keys(obj).length === 0
    );
    const hasEmptyObjectR2 = readP2.filter(
      (obj) => Object.keys(obj).length === 0
    );
    const hasEmptyObjectR3 = readP3.filter(
      (obj) => Object.keys(obj).length === 0
    );
    if (
      testname === "" ||
      ...
      alert(
        "Action cannot be done! You have not input enough questions and datas of a test."
      );
      return;
    }
    const data = {
  ...
  return (
    <div className="p-4">
      <div className="d-flex flex-row justify-content-center">
        ...
        <Button onClick={Submit} className="text-white bg-primary ms-4">
          Create test
        </Button>
      </div>
